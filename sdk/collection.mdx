...
On Hydrogen/Oxygen, pass config explicitly (there's no `process.env`): `fetchCollectionBundle(handle, { token: context.env.AGENTSHOP_TOKEN, baseUrl: context.env.AGENTSHOP_SEO_URL })`.
