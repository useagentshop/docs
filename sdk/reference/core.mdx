...
Maps a bundle to a Next.js `Metadata` object. `og:type: "product"` is deliberately omitted — it isn't in Next's `OpenGraph` type union and throws at runtime — but the remaining Open Graph tags still render with `property="og:*"`. (Also re-exported from `@agentshop/seo/next`.)
