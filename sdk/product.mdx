...
A typical rendered block is ~2.2 KB of valid JSON-LD, served from your Shopify SEO fields and product data. Multi-variant products are modeled the schema.org way: one `ProductGroup` with each variant as a `Product` under `hasVariant`, each carrying its own offer.
