...
All three exports accept an optional [`SeoClientConfig`](/sdk/reference/config); `AGENTSHOP_TOKEN` and `AGENTSHOP_SEO_URL` are read from `process.env` for anything you don't pass. All helpers follow the [never-throw contract](/sdk/concepts#graceful-degradation-the-never-throw-contract).
